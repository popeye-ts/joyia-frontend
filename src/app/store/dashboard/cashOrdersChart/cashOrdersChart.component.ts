...
import { getCashOrderCharts } from '@/Redux/selectors/dashboard.selector';

@Component({
  selector : 'cashOrdersChart',
  templateUrl: './cashOrdersChart.component.html',
  styleUrls: ['./cashOrdersChart.component.scss']
})
export class cashOrdersChartComponent implements AfterViewInit {
  cashOrdersData : any  ;
  ...
  constructor(private store: Store<State>  , private notifier : NotifierService){
    this.sales$ = this.store.select(getCashOrderCharts );
    let that = this;
  ...
  makeChart( ){
    ...
    let chartTooltip = {
      ...
      if ( document.getElementById("cashOrdersChart")) {
        var cashOrdersChart = (<HTMLCanvasElement>document.getElementById("cashOrdersChart")).getContext("2d");
        ...
        var myChart = new Chart(cashOrdersChart, {
          type: "LineWithShadow",
          options: {
            plugins: {
              datalabels: {
