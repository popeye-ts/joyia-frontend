...
@Component({
  selector : 'financefilters',
  templateUrl: './filter.component.html',
  styleUrls: ['./filter.component.scss']
})
export class FinanceFilterComponent implements AfterViewInit , OnInit{
  ...
  ngAfterViewInit() : void {
    ...
    fetchAccounts(){
      let that = this;
      this._accountService.getAll().subscribe(resp =>
        {
          let chooseOption = [{id : '' , text : 'All'}];
          let normalizedCategories =  resp.map(cat=>{ return {id: cat._id , text : cat.title }; });
          this.searchAccData = chooseOption.concat( normalizedCategories);
        });
    }
    //handling select 2
    selectAccChanged($event){
  ...
  dateLessThan(){
    ...
    public changedAccount(e: any): void {
        this.FilterForm.controls.account.setValue(e.value);
    }
    public changedType(e: any): void {
        this.FilterForm.controls.type.setValue(e.value);
    }
}
