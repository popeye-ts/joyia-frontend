...
export class FinanceComponent implements AfterViewInit , OnInit{
    ...
    that.dtOptions = {
				  ...
				  that.http.post<DataTablesResponse>(environment.apiUrl+'finance/AccountsDatatable' , dataTablesParameters , {})
                      .subscribe(resp=>{
                        ...
                        resp.data.forEach(finance => {
                          ...
                          let template = `<label class="custom-control custom-checkbox mb-0 d-inline-block">
                                              <input type="checkbox" class="custom-control-input finance-checkbox" data="${finance._id}">
        ...
        drawCallback: function () {
          ...
          $("#financeCountDatatable .dropdown-menu a").on("click", function (event) {
            //Showing spinner
            FinanceComponent.dataLengthChanged = true;
          ...
          //Page records count
          FinanceComponent.numFinances = "Displaying " +(dtInfo.start + 1) + "-" + dtInfo.end + " of " + dtInfo.recordsDisplay + " finance rows ";
          //In case filtering is applied
          if(dtInfo.recordsTotal != dtInfo.recordsDisplay)
            FinanceComponent.numFinances += `(filtered from ${dtInfo.recordsTotal})  `;
          FinanceComponent.dataLengthChanged = false;
  ...
  get numFinances(){
    return FinanceComponent.numFinances;
  }
  get dataLengthChanged(){
    return FinanceComponent.dataLengthChanged;
  }
