...
export class OrderComponent implements AfterViewInit , OnInit {
  ...
  keyPressSubscription: Subscription;
    ...
    constructor(private http:HttpClient , private datePipe : DatePipe , private $state : StateService , 
      private keyService: KeyPressDistributionService , public _permService : PermissionService , private _eventService : StoreEventService ,
      ...
      .subscribe( (evt : StoreEvent)=>{
        console.log("Order component" , evt );
    ...
    ngOnInit(){      
      ...
      that.dtOptions = {
          ...
          dataTablesParameters.filters = that.filtersHidden ? null : that.filtersJSON;
            ...
            that.http.post<DataTablesResponse>(environment.apiUrl+'order/datatable' , dataTablesParameters , {})
                        .subscribe(resp=>{
                          // return callback({
                          ...
                          for(let i=0 ; i<resp.data.length ; i++)
                            {
                              ...
                              let template = `<a href="javascript:void(0)" style="cursor:pointer;" data='${(objAtI._id ? objAtI._id : '')}'
                                    class="badge badge-light mb-1 order-detail">
                                    View&nbsp;
                                    <span class="badge badge-light btn_order_loader spinner_${objAtI._id}" style="display:none;">
                              ...
                              let idForRow = objAtI.client && objAtI.client._id ? objAtI.client._id: objAtI._id;
                                ...
                                groupedData[idForRow].actions = groupedData[idForRow].actions ? (groupedData[idForRow].actions+template) : template;
                                //Rating system
                                // "simple-icon-refresh" ,
           ...
           columns : [ {data:'client.personal_info.name'} , {data : 'client.personal_info.phone'} ,
            ...
            {data : 'last_payment_time' , className : ''} , { data : 'actions'}] ,
          language: {
            paginate: {
              first : "<<",
              last: ">>" ,
    ...
    dtDrawCallBack(){
        ...
        OrderComponent.numOrders = "Displaying " +(api.page.info().start + 1) + "-" + api.page.info().end + " of " + api.page.info().recordsTotal + " orders";
